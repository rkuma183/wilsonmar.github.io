...
0. Click the <strong>4 results for "loadrunner" on AWS Marketplace</strong>.
   ...
   FUTURE:
   "LoadRunner_12-55_CE_Windows-2016x64_50_Users" contains all components of
...
0. If you select <strong>Terminate</strong>, you will need to go through the steps to create a new instance. The .rdp and .pem files downloaded are no longer valid.
...
0. Right-Click to Download "Community Edition". Click the green Download button to get:<br />
...
## Create EC2 Custom AMI

1. Shut down the Windows server.
2. In EC2 Console, right-click the instance to select Instance State: <strong>Stop</strong>.
0. Click "Yes, Stop" to the warning.

0. Right-click the instance to select Image: Create Image.

0. For Image Name: such as "LoadRunner_12-55_CE_Windows-2016x64_50_Users".
0. For Image description: paste the URL such as `https://wilsonmar.github.io/loadrunner-12.55-install/`
0. Click "Create Image".

0. Highlight the AMI name such as "ami-fb96a29b".


## References

https://www.youtube.com/watch?v=QhOiGQ3B88A
Amazon AWS EC2 - AMI Image Creation (Windows) [2:09]
from Jul 24, 2015