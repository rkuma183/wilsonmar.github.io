...
This article is hands-on tutorial on how to hook up sensors to 
Amazon's IoT (Internet of Things) cloud services.
First you do, then concise yet deep commentary in PROTIPs are offered.

## Low cost IoT #

Amazon provides perhaps the lowest-cost IoT device,
selling different configurations: 

1) A <a target="_blank" href="https://www.amazon.com/dp/B01C7WE5WM">
$19.95</a>
<a target="_blank" href="https://aws.amazon.com/iot/button/">
Limited Release Programmable AWS IoT Button</a> is <strong>customizable</strong>.
Mine came August 3, 2016. A 2nd generation one became available in 2017.
...
2) That customizable's price seems steep compared with 
<a target="_blank" href="https://www.amazon.com/Amazon-JK29LP-Trojan-Dash-Button/dp/B01C3JBA7M/">
...
4) For bulk orders with custom artwork, use <a target="_blank" href="https://aws.amazon.com/blogs/aws/introducing-the-aws-iot-button-enterprise-program/">
...
Amazon provides services to help others include Dash into their products:

   https://developer.amazon.com/dash-services

Those without a subscription can just read the documentation:

   https://docs.aws.amazon.com/iot/latest/developerguide/configure-iot.html

or read the lab instructions below.

<a name="Config"></a>

### Configure using mobile App  #

Dash buttons are pre-defined for specific uses. Use the Amazon mobile app to configre them:
   ...
   * <a target="_blank" href="https://www.youtube.com/watch?v=oIPsQhStbnY">
   VIDEO: How to configure an AWS IoT Button</a>  Jun 11, 2017
   <br /><br />
...
2. In Play Store or Apple Store, search for <strong>AWS BTN Dev</strong>.

   ### Create and use IAM credentials

3. Login to the AWS Console at https://console.aws.amazon.com/console/home
3. If you are new to AWS, follow <a target="_blank" href="https://wilsonmar.github.io/aws-iam">these instructions to create a new AWS account root account and configure security for master billing</a>.
3. Create an AWS User account to configure the device and sign in with that.
3. <a target="_blank" href="https://docs.aws.amazon.com/iot/latest/developerguide/iot-create-role.html">
   NOTE: Add this role</a>
   
3. In the app: Click "Login" and provide your AWS IAM email and password.
 
   There is a link to "Sign in using root account credentials", but it's strongly not recommended.

4. Notice "Oregon (us-west-2)" at the bottom of the page. 
   Tap the region to select <a href="#Region">other regions support the button</a>.
5. Click "Setup AWS IoT Button".
6. Press "+" at the upper-left corner if you see a list of "AWS IoT Buttons".
7. If one is shown, slide the button to your left to reveal options:
...
7. Click "Agree & get started".
8. Click "Tap here to scan DSN barcode" and hold the barcode of the box or the back of the device within the border lines. Hold it still. It will advance to the next screen on its own.
9. PROTIP: You can change the Friendly NAME automatically assigned to something unique to you.
8. Press the blue "Register Button". 
9. Do as it requests and press the AWS IoT button for 5 seconds until a blue blinking light appears.
10. Press "Copy password and go to settings" to be sent to the Settings page.
11. In the Settings page, press "Wi-Fi" and select "Amazon ConfigureMe" while the blue light appears.
12. Select the wi-fi network both your phone and the device will be using.
12. Click on the Password entry field and Paste from before. (You don't need to remember the password).
14. Return to the previous screen by swiping left on the bottom of your iPhoneX.
15. Click Join. The light on the thing should flash when your Wi-Fi starts with "Button ConfigureMe".<br />
    But if you took too long, cancel and start over.

    BLAH: "No internet connection" appears.

11. In a browser, navigate to <a target="_blank" href="http://192.168.0.1/index.html">
   http://192.168.0.1/index.html</a>

12. The first email sent requests permission for the addressee to receive email.
...
Hello from your IoT Button G030MD040201684S. Here is the full event: {"batteryVoltage": "1594mV", "serialNumber": "G030MD040201684S", "clickType": "SINGLE"}
