...
Since the above command is done within Amazon's landscape,
it can automatically inform Amazon's IoT Resources database.

Amazon's IoT Button is authenticated a different way,
as if it were a remote server 
with its own access to the public internet.

And instead of NodeJs running on a server within EC2,
the example here makes use of Amazon Lambda.
...
* <strong>Message broker</strong>
   ...
   Use either the MQTT protocol directly or MQTT over WebSockets to publish and subscribe. 
