...
0. Display just the filenames containing the word "foo":

   <pre><strong>
   grep -r -l "foo" .
   </strong></pre>
   
   See https://www.cyberciti.biz/faq/howto-recursively-search-all-files-for-words/


   ### Use find command to show text within files

0. Find text "foo" within files by diving recursively into folders :
...
0. Use `-print0` option to find filenames that contain spaces or other metacharacters:
   ...
   Mommy, I found it! — 15 Practical Linux Find Command Examples</a>
