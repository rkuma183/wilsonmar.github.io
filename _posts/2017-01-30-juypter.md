...
A Jupyter notebook is a web application that runs several 
separate environments containing explanatory text, math equations, code, and visualizations 
all in one easily <strong>sharable</strong> document.

Notebooks used to be called "iPython" notebooks before other languages were added.

Below is a tutorial is a guided learning experience.
...
0. Create a conda environment based on the requirements file:

   <pre>conda install --file requirements.txt</pre>

0. Create a stand-alone environment named PDSH with Python 3.5 and all the required package versions, run the following:
...
0. In a new Terminal window, enter Jupyter:
