...
git config --global user.user "wilsonmar"
   ...
   (The 3rd line is not often seen. It's used to identify the user name in GitHub.)
...
Oh, there's one more thing, to make specific commits easier to find,
a tag command is added before you commit so that it travels with that commit:
...
Tags are pushed using its own command:
