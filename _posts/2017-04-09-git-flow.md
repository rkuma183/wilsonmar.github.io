...
The <strong>git status</strong> command details the status of changes to the repo. You can change several files, but only the files you add to Git's staging area will be pushed to GitHub. The dot selects all files changed. The dash capital A parameter specifies that deleted files be processed. But many prefer to specifically add individual files to go into each particular commit, which works on all files added to staging. 
...
Before pushing to GitHub, some prefer to rebase to squash some commits so that only one commit message appear for several commits made.
