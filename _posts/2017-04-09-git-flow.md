...
excerpt: "Here is the fastest way to learn to really learn Git for enterprise use"
...
This aims to be the fastest way to really getting Git to interact with GitHub repositories for enterprise use.

The video provides the sequence of commands to a "Git Flow" workflow 
for someone with a Git command-line client installed and want to work on a Git repository she cannot directly update.

The narrative text from the video is shown below the video, with additional alternatives.
...
Let's get hands-on typing each of the commands in the sequence presented.
