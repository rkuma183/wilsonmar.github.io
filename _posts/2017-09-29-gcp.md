...
* want to use GPUs (Graphic Processing Units)
* custom OS kernels
...
[Kubernetes](/kubernetes/) is Google's container orchestration manager, providing services above Google Compute Engine (GCE) are used to provide computing resources in the cloud. 

GKE separates apps from operating systems.

You set CPU, memory, and storage requirements for each <strong>node</strong>.
...
0. View cloned source code for changes
0. Cloud Shell instance - Reove code placeholds
0. Cloud Shell instance - package app into a Docker container
0. Cloud Shell instance - Upload the image to Container Registry
0. Deploy app to cluster
