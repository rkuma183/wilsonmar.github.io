...
0. Name the instance test-apache-1 (numbered in case you'll have more than one).
0. Set the zone to your closest geographical location (us-west-1a).
0. Set machine type to `f1-micro`.
0. Set Boot Disk to `Ubuntu 16.04 LTS`.
0. Allow HTTP & HTPPS traffic.
0. Add to Start-up script to install Java, Docker, etc.

0. Click Create the instance. This takes a few minutes.
...
0. Install utlities such as Java.
0. Install Docker: `curl -fsSL https://get.docker.com/ | sh`
0. Download: `sudo docker pull sonarqube`
0. Run: `sudo docker -it sonarqube` or

   `sudo docker run -d --name sonarqube -p 9000:9000 -p 9092:9092 sonarqube`

0. If there is a UI, switch back and highlight and copy the <strong>external IP address</strong> (such as https://35.203.158.223/) and paste on a browser Address.
0. Connect to your instance via <strong>external IP address</strong> in web browser.

0. Open another SSH to view processes using `ps -al`

0. Shut down the instance via terminal command.
0. Delete your instance.
