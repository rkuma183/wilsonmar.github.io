...
0. In the <strong>Startup script</strong> field, paste:
...
0. Copy and paste it in a text editor to save the command for troubleshooting later.
   ...
   TODO: Where to see instance console history?
...
0. Download: `sudo docker pull sonarqube`
   ...
   See https://docs.docker.com/samples/library/sonarqube/
   
0. Click the gear at the upper-right corner to select Text Size, Smallest.
0. Verify: `sudo docker ps -a`
0. Run: `sudo docker -it sonarqube` or

   `sudo docker run -d --name sonarqube -p 9000:9000 -p 9092:9092 sonarqube`

0. Switch back to the Google Console.

0. Click on the three dots to select <strong>View Logs</strong>.

   https://cloud.google.com/compute/docs/activity-logs

0. Open another SSH window to view processes using `ps -al`
0. View the start-up script at:
