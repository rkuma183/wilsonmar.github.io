...
0. Click on the changed file to see its text.
0. Click on the <strong>Diff against</strongh>.

   To disconnect a hosted repository:

0. Click Repositories on the left menu.
0. Click the settings icon (with the three vertical dots to the far right) on the same line as your repo.
...
0. Grant Default ACL (Access Control List) to All users:
...
BigQuery analytics database streams data at 100,000 rows per second. Automatic discounts for long term data storage. See Shine Technologies.
...
Dataflow - stream & ETL batch processing  - unified and simplified <strong>pipelines</strongh> in Java and Python
