...
## Hooks by Event
   ...
   * <a href="#ClientHooks">client hooks</a> (on your laptor or server) and 
   * <a href="#ServerHooks">server hooks</a> (on GitHub, GitLab, etc.).
...
### On the client
   ...
   1. The <strong>pre-commit</strong> hook is called <strong>before</strong> obtaining the proposed commit message to check the commit itself (rather than the message), to run linters and unit tests on the local laptop. An exit with anything other than zero aborts the commit. 

   2. The <strong>prepare-commit-msg</strong> hook is invoked after receiving a git commit, just prior to firing up the commit message editor. This hook is used to edit the message in a way that cannot be suppressed. 
   A non-zero exit aborts the commit. (1 to 3) Name of the file with the commit message, the source of the commit message (message, template, merge, squash, or commit), and the commit SHA-1 (when operating on an existing commit).

   3. The <strong>commit-msg</strong> hook is invoked to adjust the message after it has been edited in order to ensure conformity to a standard or to reject based on any criteria. It can abort the commit if it exits with a non-zero value. 	(1) The file that holds the proposed message.
...
1. PROTIP: In your GitHub repository, create a "hooks" folder containing script files 
...
<a target="_blank" href="https://github.com/git/git/blob/master/templates/hooks--update.sample">update.sample</a>
   ...
   This is because Git automatically runs command files with <strong>no file extension</strong>,
...
1. PROTIP: Mouse over each link above for the lastest version at:

   <a target="_blank" href="https://github.com/git/git/tree/master/templates">
   https://github.com/git/git/tree/master/templates</a>

1. Thus, you may delete the sample files in your repo because it's best to get the latest version from GitHub.

   Alternately, rename each hook file to customize the action you want,
   such as "pre-commit" (renamed from "pre-commit.sample"). 

   ### Other Git Hook examples
  
1. PROTIP: A longer list is at Matthew Hudson's <a target="_blank" href="http://githooks.com/">
...
1. See https://git-scm.com/book/en/v2/Customizing-Git-An-Example-Git-Enforced-Policy


## Demo on git-utilities

The git-utilities repository has been configured to demo use of local Git hooks to run 
...
10. Copy these lines and paste in the Terminal to make a change, then add and commit it for the Git Hook to take action:
   ...
   <pre><strong>
   echo "hotwilson" >>README.md
   git add .
   git commit -m"try git commit hooks"
   </strong></pre>

   You should see a "hello from" message from each of the hooks fired.

   Each of the demo hook files contains the bare minimum. For example:
...
exit 0
   ...
   Adding in logic to do useful work is outside the scope of my life ;).

11. See https://longair.net/blog/2011/04/09/missing-git-hooks-documentation/
