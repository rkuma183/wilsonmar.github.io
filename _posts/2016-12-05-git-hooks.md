...
3. Delete the sample files in your repo because it's best to get the latest version from GitHub.
   ...
   * <a href="#PowerShell">PowerShell</a> (see below)
   ...
   NOTE</a>: The minimal Python hook script:
...
8. Optionally, to enable updates from the original repo:
   ...
   ### Verify adherance to standards

   PROTIP: Some organizations impose a set of hook files on its repositories in order to impose some standards, such as:
   ...
   * etc.
   ...
   <a target="_blank" href="http://omerkatz.com/blog/2013/5/23/git-hooks-part-2-implementing-git-hooks-using-python">
   NOTE</a> Git recognizes looks for hooks in a global template folder applicable to all repositories
...
Pre and post hooks invoked upon:
