...
<img alt="git hooks diagram 650x251" src="https://cloud.githubusercontent.com/assets/300046/26558599/1ced0780-4478-11e7-93da-95db47b5eaf9.png">(click for full screen image)</a>
   ...
   1. The <strong>pre-commit</strong> hook is called <strong>before</strong> obtaining the proposed commit message to check the commit itself (rather than the commit message), to run <strong>linters and unit tests</strong> on the local laptop. An exit with anything other than zero aborts the commit. This program has access to the commit date/time, author name and email.
   ...
   This would have access to the commit SHA-1 (when operating on an existing commit).
