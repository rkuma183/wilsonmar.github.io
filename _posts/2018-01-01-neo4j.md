...
Unlike SQL databases from Microsoft, Oracle, MySQL, etc. which need commands to join physical tables together, Neo4j is a "graph" database that from the outset connects data values (nodes) in relationship to other nodes. 
...
The two nodes (data records) both are labeled of entity type "Person".
The arrow in the relationship line points from the "CHILD_OF" in the direction of the parent.
A named property of this relationship is when the relationship was established (in "2002").

This approach of building in <strong>"meaning"</strong> has proven useful for use cases such as recommendations, network/IT analysis, fraud detection, Internet of Things(IoT), and more.  

The advantage of Neo4j appears when we work with complex indirect relationships.
...
was open-sourced in 2017 under The Linux Foundation, with participants from Google, Hortonworks, IBM, Amazon, Expero, GRAKN.AI, etc.
...
2. Know Enterprise features:

   Neo4j installers include Desktop and Enterprise Edition for Developers.
   The Enterprise Edition (which requires buying a license unless the application built on top of it is open-sourced) unlocks limitations, allowing for LDAP role-based and subgraph access control, lock manager, clustering, hot backups, and monitoring. 
   See https://neo4j.com/startup-program/?ref=developers

   ### Cloud instances

   PROTIP: If you value your data, instead of hacking your way, pay a SaaS vendor such as <a target="_blank" href="https://graphstory.com/">GraphStory.com</a>.
...
7. Open the Neo4j Browser client.
   ...
   By default the Docker image for Neo4j exposes three ports for remote access:

   * 7474 for HTTP
   * 7473 for HTTPS
   * 7687 for Bolt

   PROTIP: In the center detail panel, in the Host field, notice Neo4j uses the <strong>bolt</strong> binary protocol (instead of "http") to communicate with the Neo4j database.
...
3. Click the gear icon near the lower-left corner among menu icons.
   ...
   <img width="1174" alt="neo4j-console" src="https://user-images.githubusercontent.com/300046/34465151-4dc9ded8-ee5e-11e7-87d8-e3a92d459795.png">
...
11. Click "Operations Manual" under Useful Resources to pop up a new browser tab to the version installed:
...
In Neo4j, everything is stored in the form of edges (relationships),  nodes (vertices) with attributes. Each node and edge can have any number of attributes. Both the nodes and edges can be labelled. Labels can be used to narrow searches. 
...
BTW in academic communities, relationships are also called "edges" and nodes called "____".
...
While in Seattle, William will also be giving the following Graph Day Seattle presentation: Applying an active learning algorithm for entity de-duplication in graph data
