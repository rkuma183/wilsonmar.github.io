...
Unlike SQL databases from Microsoft, Oracle, MySQL, etc. which need commands to join physical tables together, Neo4j is a "graph" database that manages connection of nodes (data entities) in relationship to other nodes. 
...
Mar 23, 2017,
Dave Bechberger (@bechbd, http://graph.experolabs.com/) explains that with Neo4j, one can easilly "travese" a graph with arbitrary hops such as "similar":
...
The approach of graph databases build in <strong>"meaning"</strong> within data. 
