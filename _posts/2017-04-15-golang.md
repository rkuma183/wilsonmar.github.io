...
0. Create a <strong>src</strong> folder in gopkgs.

   The pkg folder holds installed package objects, such as "linux_amd64".

0. Within src create a <strong>github.com</strong> folder there.
...
0. Create a $GOPATH environment variable to store a list of folders containing Go programs,
   ...
   <pre>export GOPATH=$HOME/gopkgs:$HOME/gits/wilsonmar/golang-samples</pre>
...
0. For each 3rd-party library, 
   create a folder for each GitHub user. For example, stretchr.
...
0. Clone a package repo from GitHub:
