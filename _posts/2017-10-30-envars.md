...
Information likely to vary between deploys (staging, production, developer environments, etc) include:
...
An alternative to operating system enviornment variables is writing program code to read a file. But that means they are static across multiple instances, or a mechnism needs to be crafted to change the contents of the file before use.


## MacOS 

1. On MacOS Terminal, the path of folders where the operating system searches for executables is defined in an environment variables, listed by this command:
...
In the traditional cmd.exe doesn't have its own variables. It only knows about environment variables. To display the enviornment path:
...
3. If the program cannot run less a custom variable is set, throw an exception:
...
4. A different method is used to read Java system properties:
...
Further down the file:
  ...
  switch (env.BRANCH_NAME) {
   ...
   The <strong>sed</strong> utility iteratively replaces image names.
