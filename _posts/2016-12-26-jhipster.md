...
0. <a href="#LogoConfig">Customize text and images displayed</a>, then <a href="#BrowserSync">Browser Sync</a>.
...
0. Click on Administrator to select <strong>Logs</strong>.
   ...
   Let's now take a look into the <a href="#Processes">processes</a>, servers, and software components.

   <a name="AdminConfig"></a>

   ### Admin Config

0. Click on Administrator to select <strong>Configuration</strong>.
   ...
   BLAH: Highlight all text on the page. QUESTION: <a href="#AngularDebug">why are properties hidden using a white font?</a>
...
0. Right-click on a "white" part of the web page and select <strong>View Page Source</strong>.
   ...
   PROTIP: This means that traditional software such as Selenium and LoadRunner that examine HTML and CSS may not work well with this.
...
0. Right-click on a "white" part of the web page and select <strong>Inspect</strong> to use the browser's debugger.
   ...
   `<jhi-main ng-version="4.3.2">` near the top means that Angular version 4 is being used.
...
0. Click the <strong>Account</strong> link at the upper-right corner to <strong>Sign In</strong>.
   ...
   ## Entities in Great Big Example

0. Click the <strong>Entities</strong> menu text.

   Note there are no entities defined yet in the default app generated.

   But the sample app created based on JHipster by <a target="_blank" href="https://github.com/dancancro">Dan Cancro</a> has Entities and Features:
...
0. View his "great big example" app running all the time on Heroku at:
