...
0. Create the node_modules folder (895.6 MB):

   <tt><strong>yarn install
   </strong></tt>

   NOTE: We're not sure how to eliviante several warnins that appear.
   Things run OK in spite of them so we just live with them. 
...
0. Let's dive into the Java code:
   ...
   `web` holds web and websocket DTO (Data Transfer Object) network resource code.
