...
Add another entity and JHipster builds a form screen to manage the data:
   ...
   * HTML5 Boilerplate to avoid inconsistencies among browsers.
   ...
   <a target="_blank" href="http://angularjs.blogspot.com/2016/12/ok-let-me-explain-its-going-to-be.html">AngularJS.org</a>. #Angular covers the other versions at angular.org:
...
JHipster should really be called a <strong>project generator</strong> and not just a code generator because it creates code for the entire application and also the whole <strong>DevOps stack</strong>:
   ...
   technologies underneath are <strong>changeable</strong>. You can choose between any of the popular databases:
...
## DevOps Back-end
   ...
   The "J" in JHipster refers to the Java programming language coded with Spring Boot <a target="_blak" href="https://jhipster.github.io/microservices-architecture">"microservice architecture"</a> that runs Netflix’s OSS (Open Source Software) with Docker containers.
...
## Yarn package manager used by JHipster
...
Although yarn can be installed using NPM, that's <a target="_blank" href="https://www.pluralsight.com/guides/node-js/yarn-a-package-manager-for-node-js">not recommended</a>. On Windows, <a target="_blank" href="https://yarnpkg.com/latest.msi">Download the msi installer file</a>. On Unix, use a curl command:

   <tt><strong>curl -o- -L https://yarnpkg.com/install.sh | bash
   </strong></tt>

Yarn automatically creates a <strong>yarn.lock</strong> file saved with other assets in the repo. The file ensures that the same version of every single file and the exact same file structure of the node_modules directory is the same across all machines. By contrast, NPM's `npm-shrinkwrap.json` is generated manually by the `npm shrinkwrap` command. 
