...
The Arconis backup program makes <strong>incremental</strong> backups of just the changes,
but it doesn't provide fall-back to <strong>different versions</strong> of the same file at different points in time.

This is also the case with programs to access cloud storage such as <strong>Dropbox</strong</a>
...
Network Attached Storage (abbreviated NAS) for the home or small business
operate over <strong>wi-fi</strong>, so you don't have it remember to plug it in.
It runs automatically on a <strong>schedule</strong> such as once an hour,

Some are available with two disk drives which mirror each other
for redundancy in case one of the drives break down.

### Social media backup #

One interesting feature of <a href="#SeagateNAS">Seagate's NAS software</a> 
