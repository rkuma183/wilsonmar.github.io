...
high performance, yet low memory usage.
In contrast, Apache spawns a new thread for each new connection.
...
NGINX serves as a cache, provides load balancing of incoming traffic
as a "reverse proxy".
...
It operates at "Layer 7", meaning the load balancer
examines the content of messages from applications
(URLs and cookies) to 
reuse existing connections kept alive and route directly to a specific type of server.
...
and those sending traffic use that information to send load to the server.

NGINX handles SMTP, POP3, IMAP, as well as web HTTP/HTTPS protocols.
