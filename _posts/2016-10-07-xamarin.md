...
0. Click Order By: DOWNLOADS to see what people actually use.
   ...
   0. Android Support Library for backward compatitibility
   0. Android Support Design Library

   0. AdvexpSettings for cross-platform settings, from among Libraries.

   0. Xamarin.Auth by Auth0 for 2-factor authentication
   0. Xamarin.Social for Facebook, Flickr, Twitter

   0. RESTSharp REST client for consuming HTTP APIs
   0. Amazon S3 storage among Cloud components.

   0. SQLite.NET for ORM-based local storage using SQL commands

   0. Stripe for mobile payments. Adcash for mobile ads, from among Libraries.
   0. Notifications: https://github.com/Redth/PushSharp

   0. AI.API for speech recognition and natural language processing, from among Cloud components.
   0. Google Cardboard VR (Virtual Reality) for Android (not iPhones), from among Games.
