...
Pact is a word (not an acronym) for a family of software <strong>testing code frameworks</strong> (at <a target="_blank" href="https://docs.pact.io/">https://docs.pact.io</a>) to verify whether "Consumer Driven Contract" (CDC) definitions are actually satisfied by the collaborating services built. 
...
5. The `pact.setup()` command creates a <strong>mock service Provider</strong> with which the Consumer code communicates.
...
10. The `pact.verify()` command then verifies that all and only the interactions specified occurred and that they match. This is called after any other assertions and once per test case. 
...
12. The Provider runs verification tests against a real (live) version of its service, using the shared pact file.

   This testing determines whether Test Doubles (mocks) of microservices are a valid stand-in for the real service. 
...
15. <a target="_blank" href="https://www.infoq.com/presentations/pact">[15:30]</a> Because the Pact Broker handles the traffic, it can automatically generate a Network Diagram of relationships between services (who calls whom).
...
16. Pact commands can be get pacts to run against either the Consumer or Provider.
   ...
   <tr valign="top"><td> Provider HEAD (codebase) </td><td> a) Contract tests first release </td><td> b) Contract tests to ensure <strong>provider</strong> is backwards compatible </td></tr>
   <tr valign="top"><td> c) Provider<br />Prod<br />(codebase) </td><td> Contract tests to ensure <strong>consumer</strong> is backwards compatible </td><td> b) <em>Already tested</em> released together </td></tr>
   </table>
