...
2. The developer of the consumer service writes tests that interact with a Provider. 

   This includes a <strong>state</strong> the provider should be in, 
   the body of the request and the expected response. 
   Based on this definition ...

3. Run Pact. 

4. Pact creates and spins up a provider <strong>stub</strong> against which the test executes.

5. The output of Pact Consumer runs is one or more json files such as <a href="#SampleContract">this</a>.

6. These contracts (also called "pacts") are either a) committed to a shared Git repo; b) uploaded to a shared file storage; or c) stored by the Pact Broker application.

7. Pact files are then used to make requests by consumers to the Provider service.
...
8. The Provider runs its own verification tests against a real-life version of its service, using the shared pact file.

9. When consumer software makes calls to live Provider services,
...
Consumer Driven Contracts can be an effective service evolution ...
