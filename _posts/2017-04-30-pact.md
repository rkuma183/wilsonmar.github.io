...
10. The `pact.verify()` command then verifies that all and only the <strong>interactions</strong> specified occurred and that they match. This is called after any other assertions and once per test case. 
...
18. Recent changes to the Provider codebase should be tested against pacts for the Consumer in production.
   ...
   Postel's Law, also called Robustness Principle, says be <strong>strict</strong> in interpreting requests sent out, but responses should be <strong>loose</strong> (liberal or be accomodating) such as by allowing (not rejecting) extra keys unused by some consumers.

   See <a target="_blank" href="https://www.youtube.com/watch?v=_KrCQPR2I4s">this</a> and <a target="_blank" href="https://www.youtube.com/watch?v=1B4KjAhQJoQ">this</> video uses the example of internet browsers being forgiving of HTML errors.
