...
If you are running a Windows virtual instance (under VMWare Fusion), etc., this is available by default on all Windows machines.
...
<a href="#LinuxRDP">Linux machines have rdesktop</a>
...
A) <a href="RDC1">Microsoft's Remote Desktop Client Connection 2.1.1 for Macintosh is gone</a>
...
### Microsoft's Remote Desktop for Mac OSX 
...
1. Set the communicator and network ports in your Vagrant file, such as:

   <pre>config.vm.communicator = "winrm"</pre>

   <pre>config.vm.network "forwarded_port", host:3389, guest: 3389</pre>

0. On a Terminal:

    <tt><strong>vagrand rdp
    </strong></tt>
